...
//     auto input = readCode(CodeBin::Touch, 0);
...
//         Nextion::sendLog(NxtLogSeverity::Warning, "nexLoop err");
//     }
...
//     Nextion::sendLog(NxtLogSeverity::Debug, std::string{"TouchEvent: "} +
...
//         Nextion::sendLog(NxtLogSeverity::Warning, "TouchEvent ignored");
//     }
// }

Nextion::Nextion(nxt::Stream &port, nxt::LogCallback logCb)
    : _port(port, logCb), _logCallback(logCb) {
    _isHealthy = true;
...
void Nextion::sendLog(nxt::LogSeverity level, std::string log) {
