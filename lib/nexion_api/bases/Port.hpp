...
/// @brief Functional definition for Log-callbacks
...
/**
 * @brief Stream-wrapper class for Display interface
 * @details Made sure that class is not copyable
 */
class Stream {
  public:
    Stream() = default;
    /// @brief Read minimum size with timeout
    /// @param minSize minimum bytes to read
    /// @param msTimeout timeout
    /// @return read bytes, incomplete in case of timeout
    virtual std::vector<uint8_t> read(size_t minSize = 0, unsigned msTimeout = 0) = 0;

    /// @brief Write character to port
...
/**
 * @brief Types of messages in order to sort them in message bins
 */
enum class CodeBin : uint8_t {
    /// default, unknown or not implemented
    Trash = 0,

    /// return type, acknowledge for command
    Return,
    ...
    /// Helper for bin-limit (last entry)
    BinsCount,
};

/**
 * @brief Protocol port for display
 */
class Port {
  public:
    /// @brief Constructor
    /// @param port reference to serial port/stream
    /// @param logCb Callback handler for logging
    ...
    /// @param payload command payload as string
    void sendCommand(std::string payload);

    /// @brief Read code with timeout
    /// @param code awaited code
    /// @param minPayload hint for payload size
    /// @param timeout timeout in ms
    /// @return received buffer, empty in case of timeout
    Buffer readCode(CodeBin code, int minPayload, unsigned msTimeout);
