...
bool Rtc::write_rtc_time(uint32_t time[6]) {
    ...
    _port.sendCommand(cmd);
    ret &= _port.recvRetCommandFinished();
...
bool Rtc::read_rtc_time(char *time, uint32_t len) {
    ...
    cmd = "get rtc0";
    _port.sendCommand(cmd);
    year = _port.recvRetNumber();

    cmd = "get rtc1";
    _port.sendCommand(cmd);
    mon = _port.recvRetNumber();

    cmd = "get rtc2";
    _port.sendCommand(cmd);
    day = _port.recvRetNumber();

    cmd = "get rtc3";
    _port.sendCommand(cmd);
    hour = _port.recvRetNumber();

    cmd = "get rtc4";
    _port.sendCommand(cmd);
    min = _port.recvRetNumber();

    cmd = "get rtc5";
    _port.sendCommand(cmd);
    sec = _port.recvRetNumber();

    cmd = "get rtc6";
    _port.sendCommand(cmd);
    week = _port.recvRetNumber();
...
bool Rtc::read_rtc_time(uint32_t *time, uint32_t len) {
    ...
    cmd = "get rtc0";
    _port.sendCommand(cmd);
    ret &= _port.recvRetNumber(time_buf[0]);

    cmd = "get rtc1";
    _port.sendCommand(cmd);
    ret &= _port.recvRetNumber(time_buf[1]);

    cmd = "get rtc2";
    _port.sendCommand(cmd);
    ret &= _port.recvRetNumber(time_buf[2]);

    cmd = "get rtc3";
    _port.sendCommand(cmd);
    ret &= _port.recvRetNumber(time_buf[3]);

    cmd = "get rtc4";
    _port.sendCommand(cmd);
    ret &= _port.recvRetNumber(time_buf[4]);

    cmd = "get rtc5";
    _port.sendCommand(cmd);
    ret &= _port.recvRetNumber(time_buf[5]);

    cmd = "get rtc6";
    _port.sendCommand(cmd);
    ret &= _port.recvRetNumber(time_buf[6]);
